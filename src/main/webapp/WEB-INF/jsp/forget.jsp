...
<!--兼容IE-->
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/QQfiles/main.css">
...
<script>
	$(function (){
		$("input[id='input_find_qq']:first").focus();
		$("#tipdiv_input_find_qq").attr("style","display: none;");
		$("#input_find_qq").focus();
		$("#input_find_qq").val("");
		$("#verifycode").val("");
	...
	//
	function nofocus(){
		$(document).on("blur", "#input_find_qq", function(){
			if($(this).val().length==0){
				$("#tipdiv_input_find_qq").attr("style","");
			}
			if($(this).val().length!=0){
				$("#tipdiv_input_find_qq").attr("style","display: none;");
			}
	...
	function next_stepOnclick(){
		$('#next_step').click(function(){
			var verifycode = $('#verifycode').val().toUpperCase();
			var input_find_qq = $('#input_find_qq').val();
			if(input_find_qq ==''){
				$("#tipdiv_input_find_qq").attr("style","");
				return false;
			}
			if(verifycode == '' ){
				 $("#tipdiv_input_verifycode").attr("style","");
				return false;
			}else{
				$("#tipdiv_input_verifycode").attr("style","display: none;");
			}
			$.getJSON("${pageContext.request.contextPath}/forget/validate", {"yanzhen": verifycode}, function(result){
				 if(result.status=="success"){
					 	var input_find_qq = $('#input_find_qq').val();console.log(input_find_qq);
						window.location = "${pageContext.request.contextPath}/forget/tovalidate/"+input_find_qq;
				 }
				 if(result.status=="error"){
					 tipdiv_input_verifycode
					 $("#tipdiv_input_verifycode").attr("style","");
				 }
			});
        });
	...
	function validate(){
		$('#re_get_verifycode_image').click(function(){
        	$('#imgVerify2').attr("src","${pageContext.request.contextPath}/forget/captcha?timestamp=" + (new Date()).valueOf());
        });
	};
</script>
		...
		<div>
			...
			<div class="section section-main">
				<div class="section_main" id="container">
							...
							<div class="mod-form-block input">
								...
								<div>
									<div class="formblock medium" style="width: 264px">
										...
										<input type="text" style="width: 264px" id="input_find_qq"
											placeholder="QQ/邮箱/手机号/微博帐号" class="tips-error-border">

										<!--
                                <input type="text" id="input_find_qq" placeholder="QQ/邮箱/手机号">
                                -->
									</div>
									<div class="tips-error-text" id="tipdiv_input_find_qq" style="display: block;">
										<div>
											<span id="tipword_input_find_qq">不能为空</span>
										</div>
									</div>
								</div>
							</div>
							<div class="mod-form-block input verify">
								<label for="code-verify">验证码：</label>
								<div>
									<div class="formblock medium">
										<input type="text" id="verifycode" placeholder="填入显示的字符，不区分大小写">
									</div>
									<div class="tips-error-text" style="display: none"
										id="tipdiv_input_verifycode">
										<div>
											<span id="tipword_input_verifycode">您输入的验证码有误，请重新输入</span>
										</div>
									</div>
									
									<div class="verifyblock">
										<img src="${pageContext.request.contextPath }/forget/captcha" width="130" height="53" alt="" id="imgVerify2">
										<a id="re_get_verifycode_image" href="javascript:void(0)">看不清，换一张</a>
									</div>
									<div class="tips-error-text" id="tipdiv_verifycode"
										style="display: none">
										<div>
											<span id="tipword_verifycode">请先进行安全验证</span>
										</div>
									</div>
								</div>
							</div>
							<a class="mod-btn blue large"
								href="###"
								id="next_step" style="margin-top: 10px"><span>确定</span></a>
						</form>
					</div>

					<div class="mod-popup" id="mod_popup">
						<div class="popup_mask"></div>
						<div class="popup_body">
							<div class="body_content">
								<div class="content_text">
									<p class="edit-tips">您打算修改的是哪一类密码</p>
									<ul class="edit-account-list">
										<li><a
											href="https://aq.qq.com/v2/uv_aq/html/reset_pwd/pc_reset_pwd_input_account.html?v=4.0&old_ver_account=#"
											id="qq_choice_click"> <span class="type-icon qq"></span>
												<span>QQ帐号的密码</span> <span class="link-arrow"></span>
										</a></li>
										<li><a
											href="https://aq.qq.com/v2/uv_aq/html/reset_pwd/pc_reset_pwd_input_account.html?v=4.0&old_ver_account=#"
											id="txwb_choice_click"> <span class="type-icon txwb"></span>
												<span>腾讯微博的密码</span> <span class="link-arrow"></span>
										</a></li>
									</ul>
								</div>
							</div>
					...
					<div class="mod-popup warning" id="mod_popup_warning_div">
						<div class="popup_mask"></div>
						<div class="popup_body">
							<div class="body_content">
								<div class="content_pic"></div>
								<div class="content_text">
									<p class="text_title" id="warn_tip_word">系统处理超时，请稍后重试</p>
								</div>
							</div>
							<div class="body_btns">
								<a class="mod-btn blue large" id="warn_tip_btn"><span>确定</span></a>
							</div>
							<div class="i-close"></div>
						</div>
					</div>
