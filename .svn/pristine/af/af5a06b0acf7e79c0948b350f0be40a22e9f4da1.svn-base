		...
		<div>
			...
			<div class="section section-main">
				<div class="section_main" id="container">
					<div class="main_form friends">
						<form id="input_account_form" action="${pageContext.request.contextPath}/forget/question" method="post">
							<div class="mod-form-block">
								<input type="hidden" name="username" value="${username}">
								<select name="dicId1" style="width: 100%;height: 36px;border-radius: 2px;border: 1px solid #9aa0a7;line-height: 36px;text-indent: 10px;fontsize: 14px;background-color: #fff;-webkit-transition: border-color .2s,box-shadow .2s;transition: border-color .2s,box-shadow .2s;">
									<c:forEach var="dic" items="${dic}">
										<option value="${dic.dicid}">${dic.dicvalue}</option>
									</c:forEach>
								</select>
								<label>第一问题：<span class="tips-label">（必填）</span></label>
								<div class="formblock">
									<input type="text" id="arw1" name="arw1">
								</div>
							</div>
							
							<div class="mod-form-block" >
								<select name="dicId2" style="width: 100%;height: 36px;border-radius: 2px;border: 1px solid #9aa0a7;line-height: 36px;text-indent: 10px;fontsize: 14px;background-color: #fff;-webkit-transition: border-color .2s,box-shadow .2s;transition: border-color .2s,box-shadow .2s;">
									<c:forEach var="dic" items="${dic}">
										<option value="${dic.dicid}">${dic.dicvalue}</option>
									</c:forEach>
								</select>
								<label>第二问题：<span class="tips-label">（必填）</span></label>
								<div class="formblock" >
									<input type="text"id="arw2" name="arw2">
								</div>
							</div>
							<div class="mod-form-block">
								<select name="dicId3" style="width: 100%;height: 36px;border-radius: 2px;border: 1px solid #9aa0a7;line-height: 36px;text-indent: 10px;fontsize: 14px;background-color: #fff;-webkit-transition: border-color .2s,box-shadow .2s;transition: border-color .2s,box-shadow .2s;">
									<c:forEach var="dic" items="${dic}">
										<option value="${dic.dicid}">${dic.dicvalue}</option>
									</c:forEach>
								</select>
								<label>第三问题：<span class="tips-label">（必填）</span></label>
								<div class="formblock">
									<input type="text" id="arw3" name="arw3">
								</div>
							</div>
							<div style="text-align: center;">
								<input type="submit" value="提交" style="background-color: #2f86f6;border-radius: 2px;min-width: 120px;cursor: pointer;min-width: 120px;line-height: 37px;font-size: 16px;color: #fff;box-shadow: 0 3px 0 #266ecb;">
							</div>
						</form>
					</div>
				</div>
